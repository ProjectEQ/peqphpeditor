...
switch ($action) {
  case 0: // Default View
    ...
    break;
  case 4: // Dynamic Zone Details
    $body = new Template("templates/expeditions/expeditions.details.tmpl.php");
    $breadcrumbs .= " >> Dynamic Zone Details";
    $body->set('yesno', $yesno);
    $body->set('dynamic_zone_type', $dynamic_zone_type);
    $body->set('zoneids', $zoneids);
    $dynamic_zone_lockout = get_dynamic_zone_lockout_by_dynamic_zone_id($_GET['id']);
    if ($dynamic_zone_lockout) {
      $body->set('dynamic_zone_lockout', $dynamic_zone_lockout);
    }
    ...
    break;
  case 7: // View Dynamic Zone Lockouts
    $body = new Template("templates/expeditions/expedition.lockouts.view.tmpl.php");
    $breadcrumbs .= " >> Dynamic Zone Lockouts";
    $dynamic_zone_lockouts = get_dynamic_zone_lockouts();
    if ($dynamic_zone_lockouts) {
      $body->set('dynamic_zone_lockouts', $dynamic_zone_lockouts);
    }
    break;
  case 8: // Add Dynamic Zone Lockout
    check_authorization();
    $body = new Template("templates/expeditions/expedition.lockout.add.tmpl.php");
    $breadcrumbs .= " >> <a href=\"index.php?editor=expeditions&action=7\">Dynamic Zone Lockouts</a>" . " >> Add Lockout";
    $body->set('suggest_id', suggest_dynamic_zone_lockout_id());
    break;
  case 9: // Insert Dynamic Zone Lockout
    check_authorization();
    insert_dynamic_zone_lockout();
    $id = $_POST['id'];
    header("Location: index.php?editor=expeditions&action=7");
    exit;
  case 10: // Edit Dynamic Zone Lockout
    $body = new Template("templates/expeditions/expedition.lockout.edit.tmpl.php");
    $breadcrumbs .= " >> <a href=\"index.php?editor=expeditions&action=7\">Dynamic Zone Lockouts</a>" . " >> Edit Lockout";
    $dynamic_zone_lockout = get_dynamic_zone_lockout($_GET['id']);
    if ($dynamic_zone_lockout) {
      $body->set('dynamic_zone_lockout', $dynamic_zone_lockout);
    }
    break;
  case 11: // Update Dynamic Zone Lockout
    check_authorization();
    update_dynamic_zone_lockout();
    $id = $_POST['id'];
    header("Location: index.php?editor=expeditions&action=7");
    exit;
  case 12: // Delete Dynamic Zone Lockout
    check_authorization();
    delete_dynamic_zone_lockout($_GET['id']);
    header("Location: index.php?editor=expeditions&action=7");
    exit;
  case 13: // View Dynamic Zones
    $body = new Template("templates/expeditions/dynamic.zones.view.tmpl.php");
    $breadcrumbs .= " >> Dynamic Zones";
    $body->set('dynamic_zone_type', $dynamic_zone_type);
  ...
  case 14: // Add Dynamic Zone
    ...
    $breadcrumbs .= " >> Dynamic Zones >> Add Zone";
    $body->set('dynamic_zone_type', $dynamic_zone_type);
    $body->set('zoneids', $zoneids);
  ...
  case 16: // Edit Dynamic Zone
    ...
    $breadcrumbs .= " >> Dynamic Zones >> Edit Zone";
    $body->set('dynamic_zone_type', $dynamic_zone_type);
    $body->set('zoneids', $zoneids);
    $dynamic_zone = get_dynamic_zone($_GET['id']);
    if ($dynamic_zone) {
  ...
  case 19: // View Dynamic Zone Members
    ...
    $breadcrumbs .= " >> Dynamic Zone Members";
    $body->set('yesno', $yesno);
  ...
  case 20: // Add Dynamic Zone Member
    ...
    $breadcrumbs .= " >> <a href=\"index.php?editor=expeditions&action=19\">Dynamic Zone Members</a>" . " >> Add Member";
  ...
  case 22: // Edit Dynamic Zone Member
    ...
    $breadcrumbs .= " >> <a href=\"index.php?editor=expeditions&action=19\">Dynamic Zone Members</a>" . " >> Edit Member";
  ...
  case 25: // View Character Lockouts
    ...
    $breadcrumbs .= " >> Character Lockouts";
  ...
  case 26: // Add Character Lockout
    ...
    $breadcrumbs .=  " >> <a href=\"index.php?editor=expeditions&action=25\">Character Lockouts</a>" . " >> Add Lockout";
  ...
  case 28: // Edit Character Lockout
    ...
    $breadcrumbs .= " >> <a href=\"index.php?editor=expeditions&action=25\">Character Lockouts</a>" . " >> Edit Lockout";
...
function get_dynamic_zone_lockouts() {
  global $mysql;

  $query = "SELECT * FROM dynamic_zone_lockouts";
...
function get_dynamic_zone_lockout($id) {
  global $mysql;

  $query = "SELECT * FROM dynamic_zone_lockouts WHERE id=$id";
...
function insert_dynamic_zone_lockout() {
  global $mysql;
  ...
  $query = "INSERT INTO dynamic_zone_lockouts SET id=$id, dynamic_zone_id=$dynamic_zone_id, event_name=\"$event_name\", expire_time=\"$expire_time\", duration=$duration, from_expedition_uuid=\"$from_expedition_uuid\"";
  $mysql->query_no_result($query);
}

function update_dynamic_zone_lockout() {
  global $mysql;
  ...
  $query = "UPDATE dynamic_zone_lockouts SET dynamic_zone_id=$dynamic_zone_id, event_name=\"$event_name\", expire_time=\"$expire_time\", duration=$duration, from_expedition_uuid=\"$from_expedition_uuid\" WHERE id=$id";
  $mysql->query_no_result($query);
}

function delete_dynamic_zone_lockout($id) {
  global $mysql;

  $query = "DELETE FROM dynamic_zone_lockouts WHERE id=$id";
  $mysql->query_no_result($query);
}

function suggest_dynamic_zone_lockout_id() {
  global $mysql;

  $query = "SELECT MAX(id) AS id FROM dynamic_zone_lockouts";
  $result = $mysql->query_assoc($query);
...
function get_dynamic_zone_lockout_by_dynamic_zone_id($dynamic_zone_id) {
  global $mysql;

  $query = "SELECT * FROM dynamic_zone_lockouts WHERE dynamic_zone_id=$dynamic_zone_id";
  $result = $mysql->query_assoc($query);
