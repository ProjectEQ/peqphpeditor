...
<?if ($merlist != ''):?>
  <?foreach($merlist as $merlist):?>
            <td align="center" width="5%"> <a href="index.php?editor=npc&z=<?=get_zone_by_npcid($merlist['npcid'])?>&npcid=<?=$merlist['npcid']?>"> <?=$merlist['name']?> - (<?=get_zone_by_npcid($merlist['npcid'])?>)</td>
 <?endforeach;?>
<?endif;?>
<?if ($merlist == ''):?>
               <tr>
