  <div class="table_container" style="width: 750px;">
    <div class="table_header">
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td>Assassinate Adventures</td>
          <td align="right">    
            <a href="index.php?editor=adventures&z=<?=$currzone?>&zoneid=<?=$currzoneid?>&npcid=<?=$npcid?>&action=10"><img src="images/add.gif" border="0" title="Add an adventure"></a>
          </td>
        </tr>        
      </table>
    </div>
...
<?$x=0; foreach($bossassa as $bossassa=>$v):?>
...
<?if($v['type'] == 1 || $v['type'] == 4):?>
          <td align="center" width="10%"><a href="index.php?editor=npc&z=<?=get_zone_by_npcid($v['type_data'])?>&zoneid=<?=get_zoneid_by_npcid($v['type_data'])?>&npcid=<?=$v['type_data']?>"><?=getNPCName($v['type_data'])?></a></td>
<?endif;?>
<?if($v['type'] == 3):?>
          <td align="center" width="10%"><?=get_item_name($v['type_data'])?> <span>[<a href="https://lucy.allakhazam.com/item.html?id=<?=$v['type_data']?>" target="_blank">Lucy</a>]</span></td>
<?endif;?>    
<?if($v['type'] == 2):?>   
...
<? if (!isset($bossassa)):?>
        <tr>
          <td align="left" width="100" style="padding: 10px;">There are no assassinate adventures assigned to this NPC.</td>
        </tr>
<?endif;?>
      </table>
    </div>
