...
<?$export_sql = export_recipe_sql();?>
  ...
  <div class="table_container" style="width:350px;">
    ...
    <div class="table_content">
      ...
      <strong>Learn Flag:</strong> <?echo ($must_learn || $learned_by_item_id > 0) ? "Yes" : "No";?><br>
...
<? foreach ($containers as $container): extract($container);?>
          <tr>
            <td width="40%">
<?if($item_id < 1001):?>
              <?=$name?> (<?=$item_id?>)
...
<? if (isset($components) && $components != ''):?>
        <table width="100%" cellspacing="0" cellpadding="0">
<? foreach ($components as $component): extract($component);?>
          <tr>
            <td width="40%">
              <?=$name?> (<a href="index.php?editor=items&ts=<?=$ts?>&rec=<?=$rec?>&tsid=<?=$id?>&id=<?=$item_id?>&action=2"><?=$item_id?></a>)
            </td>
